...
export default function showPhotographerProfil(jsonObj) {
  ...
  var media = medias.filter(
    ({ photographerId }) => photographerId.toString() === myID
  );
  var zeroSpaceFolder = photographer.name.replace(/ +/g, "");
  // Variable globale de la fonction heartLikesHandler
  let allLikesSum = media
    .map((medi) => medi.likes)
    .reduce((acc, medi) => medi + acc);
  ...
  function displayBanner() {
    ...
    myArticleBanner.innerHTML = `<section>         
            <div class="infos">
                ...
                <ul>
                    ${tags
                      .map(
                        (tag) => `<a href="./index.html"><li>#${tag}</li></a>`
                      )
                      .join("")}                
                </ul>
            </div>
            ...
            <div class="imgWrapper">
                <img src="./images/PhotographersIDPhotos/${
  ...
  function mediaSort(event, activeElement) {
    ...
    let filterSelect = document.querySelector(
      "#portofolio > article > ul > li.topFilterInMenu"
    );
  ...
  function filterMenuDisplay() {
    ...
    listbox.addEventListener("keydown", (event) => {
      ...
      if (key === "ArrowUp")
        selectedOption = activeElement.previousElementSibling;
    ...
    hearts.forEach((heart) =>
      heart.addEventListener("click", (event) => {
        media.map((medi) => {
          ...
          let altTarget =
            event.target.parentNode.querySelector("h4").textContent;
          if (
